...
```{r}
new_grade <- readRDS('data/new_grade.rds')

food_new_score <- food_w_match_clean %>% inner_join(new_grade,by='code')
food_w_match_clean <- unique(food_new_score)
names(food_new_score) <- c("code","product_name","qwe","nutrition-score-fr_100g")


unique(food_new_score)

usethis::use_data(food_w_match_clean,marmiton_recipes,marmiton_recipes_ingredients, must_have_ingredients,overwrite = TRUE)
```

```{r}
#' Add nutrition score to food
#'
#' @param scores 
#' @param food
#' 
#' 
#'
#' @return  augmented food dataframe
#' @export
add_grade <- function(food,scores){
  
  food_new <- food %>% inner_join(scores, by='code')
  return(food_new)
